...
def train(args, gen_net1: nn.Module, gen_net2: nn.Module, dis_net: nn.Module, gen_optimizer1,gen_optimizer2, dis_optimizer, gen1_avg_param,gen2_avg_param, train_loader, epoch,
    ...
    def Cal_Orthgonal(vector1, vector2):
        dot_product = 0.0  # 两个向量的内积
        normA = 0.0
        normB = 0.0
        for a, b in zip(vector1, vector2):
            dot_product = dot_product + a * b  # 内积 = 各维度分量的乘积和
            normA = normA + a ** 2
            normB = normB + b ** 2
        if normA == 0.0 or normB == 0.0:
            return 0.0
        else:
            return abs(dot_product) / (((normA * normB) ** 0.5)+1e-8)  # A * B / ||A|| ||B||
    ...
    for iter_idx, (imgs, _) in enumerate(tqdm(train_loader)):
        ...
        real_imgs = imgs.type(torch.cuda.FloatTensor)  # [b, c, 32, 32]
       ...
       # errD_fake1.backward()
        ...
        if global_steps % args.n_critic == 0:
           ...
           #  output_G[1] = errG2_l1
            ...
            errG_MI_1.backward(retain_graph=True)  # 这里参数表明保留backward后的中间参数。可以进行下一次backward不报错。
            gen_optimizer1.step()
            errG_MI_2 = g_loss2 / 2.0 + Cal_Orthgonal(errG1_l1, errG2_l1)
            errG_MI_2.backward(retain_graph=True)
            gen_optimizer2.step()
            # learning rate
            if lr_schedulers1 and lr_schedulers2:
                gen_scheduler1, dis_scheduler = lr_schedulers1
                gen_scheduler2, dis_scheduler = lr_schedulers2
...
def validate(args, fixed_z, fid_stat, gen_net1: nn.Module,gen_net2: nn.Module, writer_dict):
    ...
    # for iter_idx in tqdm(range(2), desc='sample images'):  # 测试
...
class LinearLrDecay(object):  # 线性学习率衰减策略
