...
def main():
    ...
    # weight init
    def weights_init(m):  # 根据子模型的类型初始化参数
    ...
    args.max_epoch_D = args.max_epoch_G * args.n_critic  # 5*5 n_critic:num of training steps for discriminator per iter
    if args.max_iter_G:
        args.max_epoch_D = np.ceil(args.max_iter_G * args.n_critic / len(train_loader))  # 向上取整
    ...
    # initial
    fixed_z = torch.cuda.FloatTensor(np.random.normal(0, 1, (25, args.latent_dim)))  # 生成[25,latent_dim]的(0,1)正态分布
    ...
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch_D)), desc='total progress'):
        ...
        tau = max(0.1, args.tau_max * np.exp(-tau_decay * epoch)) if args.gumbel_softmax else None  # 温度系数τ
        if tau:
